...
public interface StockAdjustmentRepository extends JpaRepository<StockAdjustment, UUID> {
    ...
    @Query("SELECT sa.adjustmentType, COUNT(sa), COALESCE(SUM(sa.difference), 0) FROM StockAdjustment sa WHERE sa.status = 'COMPLETED' GROUP BY sa.adjustmentType")
    List<Object[]> getAdjustmentSummaryByType();

    @Query("SELECT sa.warehouse.name, COUNT(sa), COALESCE(SUM(sa.difference), 0) FROM StockAdjustment sa WHERE sa.status = 'COMPLETED' GROUP BY sa.warehouse.id, sa.warehouse.name")
