...
public interface StockAdjustmentRepository extends JpaRepository<StockAdjustment, UUID> {
        ...
        @Query("""
                            SELECT
                                ...
                                COALESCE(SUM(sa.difference),0) AS totalDifference,
                                SUM(CASE WHEN sa.difference > 0 THEN 1 ELSE 0 END) AS positiveAdjustments,
                                SUM(CASE WHEN sa.difference < 0 THEN 1 ELSE 0 END) AS negativeAdjustments
